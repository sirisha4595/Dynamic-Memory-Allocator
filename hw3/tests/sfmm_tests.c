...
Test(sf_memsuite_student, realloc_larger_block_till_ENOMEM, .init = sf_mem_init, .fini = sf_mem_fini,.signal = SIGABRT) {

	sf_errno = 0;
	void *x = sf_malloc(PAGE_SZ);

	x = sf_realloc(x, PAGE_SZ<<8);
	cr_assert_null(x, "x is not NULL!");
	cr_assert(sf_errno == ENOMEM,"sf_errno not equal to ENOMEM!!");
	sf_free(x);

}

Test(sf_memsuite_student, malloc_zero_size, .init = sf_mem_init, .fini = sf_mem_fini,.signal = SIGABRT) {
	sf_errno = 0;
	void *y = sf_malloc(0);
	cr_assert(y==NULL,"sf_malloc did not NULL!");
	cr_assert(sf_errno == 0, "sf_errno is not zero!");
	sf_free(y);
...
Test(sf_memsuite_student,invalid_address_to_realloc, .init = sf_mem_init, .fini = sf_mem_fini,.signal = SIGABRT) {
	sf_errno = 0;
	void *y = sf_malloc(sizeof(int));
	cr_assert(y!=NULL,"sf_malloc did not NULL!");
	sf_realloc((void*)((uintptr_t)y+4056),sizeof(double));//invalid address passed to sf_realloc
	cr_assert(sf_errno==EINVAL,"errno not EINVAL!!");
}

Test(sf_memsuite_student,correct_payload, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	int *y = sf_malloc(sizeof(int));
	cr_assert(y!=NULL,"sf_malloc did not NULL!");
	*y=2;
	sf_header* y_head=(sf_header*)((uintptr_t)y-sizeof(y_head->info));
	cr_assert(y_head->payload==2,"payload is not correct!!");
	sf_realloc(y,sizeof(char));
	cr_assert(y_head->payload==2,"payload is not correct!!");
	sf_free(y);
...
Test(sf_memsuite_student,invalid_allocated_field_free, .init = sf_mem_init, .fini = sf_mem_fini, .signal = SIGABRT) {
	sf_errno = 0;
	int *y = sf_malloc(sizeof(int));
	sf_header* y_head=(sf_header*)((uintptr_t)y-sizeof(y_head->info));
	y_head->info.allocated=0;//Invalid allocated field
	sf_free(y);
}